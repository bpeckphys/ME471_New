...
void main(int argc, char** argv)
{
    ...
    /* TODO : .... */
    int root = 0;
    MPI_Mcast(&tol,1,MPI_DOUBLE,root,MPI_COMM_WOLRD);
    MPI_Mcast(&a,1,MPI_DOUBLE,root,MPI_COMM_WOLRD);
    MPI_Mcast(&b,1,MPI_DOUBLE,root,MPI_COMM_WOLRD);
    MPI_Mcast(&n_global,1,MPI_DOUBLE,root,MPI_COMM_WOLRD);
    MPI_Mcast(&kmax,1,MPI_DOUBLE,root,MPI_COMM_WOLRD);

    double w = (b-a)/nprocs;
    double h = (b-a)/n_global;
    int m = n_global/nprocs; // m is number of subintervals
    ...
    zeros_array(m+3,&F);
    linspace_array(range[0]-h,range[1]+h,m+3);
